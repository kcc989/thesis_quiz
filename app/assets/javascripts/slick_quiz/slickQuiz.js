...
(function($){
    $.slickQuiz = function(element, options) {
	    ...
	    _quizOpponent          = _element + ' .quizOpponentScore',
            _quizLevel             = _element + ' .quizLevel',
	    _quizSubmit            = _element + ' .quizSubmit',
		...
		$("#submitter").on('click', function(e){
			e.preventDefault();
			console.log("This is being called. New placce");
			var result = {result: {
				...
				race: $("input[name='race']:checked").map(function () {
					return this.value=="Other" ? $("#raceOther").val() : this.value;}).get().join(", "),
				income: $('input[name="income"]:checked').val(),
				religion: $("input[name='religion']:checked").map(function () {
					return this.value=="Other" ? $("#religionOther").val() : this.value;}).get().join(", ")
			    }};
			...
			if(result["result"]["participant_id"] == ""){
			    error = true;
			    $('#idSelect').after('<span class="error"> Please input your participant ID for this study!<span>');
			}
			if(result["result"]["class_year"] == ""){
			     error = true;
			    $('#classYearSelect').after('<span class="error"> Please select your class year!<span>');
			}
			if(result["result"]["major"] == ""){
			     error = true;
			    $('#majorSelect').after('<span class="error"> Please select your major/anticipated major!<span>');
			}
			if(result["result"]["gpa"] == ""){
			     error = true;
			    $('#gpaSelect').after('<span class="error"> Please select your gpa range!<span>');
			}
			if(result["result"]["gender"] == ""){
			     error = true;
			    $('#genderSelect').after('<span class="error"> Please select your gender!<span>');
			}
			if(result["result"]["race"] == ""){
			     error = true;
			    $('#raceSelect').after('<span class="error"> Please select the race you identify with!<span>');
			}
			if(result["result"]["income"] == ""){
			     error = true;
			    $('#incomeSelect').after('<span class="error"> Please select the combined income range of your household in 2014!<span>');
			}
			if(result["result"]["religion"] == ""){
			     error = true;
			    $('#religionSelect').after('<span class="error"> Please selectt the religion you identify with!<span>');
			}
		...
		if(quizValues.info.opponent != null){
		    console.log(score);
		    console.log( quizValues.info.opponent);
	    ...
	    // Bind "submit" button
	    $(_element + ' ' + _quizSubmit).on('click', function(e){
		    e.preventDefault();
		    console.log("This is being called");
		    var result = {result: {
			    score: score,
